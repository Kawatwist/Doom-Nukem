...
void	display_slider(t_win *wn, SDL_Rect rect, SDL_Texture *texture)
{
	...
	tmp = define_rect(rect.x + rect.w + 0.25 * wn->xscreen / 8, rect.y, 0.25 * wn->xscreen / 8, rect.h);
	...
	else
	{
		...
		tmp = define_rect(rect.x + rect.w + 0.25 * wn->xscreen / 8, rect.y + move, 0.25 * wn->xscreen / 8, slider_height);
