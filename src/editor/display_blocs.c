...
static void		print_arrow(t_win *wn, t_edit *edit)
{
	...
	if (SDL_RenderCopy(wn->rend, edit->tab->fleche, NULL, &dst) < 0)
...
void		print_tbp_editor(t_win *wn, t_edit *edit)
{
	...
	edit->tab->tbp == 0 ? edit->tab->texture_tbp =
	findtexture(wn, "editor", "affichage", "texts") : 0;
	edit->tab->tbp == 1 ? edit->tab->texture_tbp =
	findtexture(wn, "editor", "affichage", "blocs") : 0;
	edit->tab->tbp == 2 ? edit->tab->texture_tbp =
	findtexture(wn, "editor", "affichage", "params") : 0;
	(edit->tab->texture_tbp == NULL) ?
	stop_exec("texture params failed in print_tbp\n", wn) : 0;
	SDL_QueryTexture(edit->tab->texture_tbp, NULL, NULL, &src.w, &src.h);
	if (edit->tab->in == 1)
	{
	...
	if (SDL_RenderCopy(wn->rend, edit->tab->texture_tbp, &src, &dst) < 0)
