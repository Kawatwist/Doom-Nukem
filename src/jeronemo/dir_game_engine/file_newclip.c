...
/*   Updated: 2019/07/21 18:11:12 by lomasse          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
static t_myvec	find_intersection(t_myvec v1, t_myvec v2, char side) // <= V2 is outside L=0, U=1, R=2, D=3
{
	...
	side == 1 ? dir.y = 200 : 0;
	side == 2 ? dir.x = 200 : 0;
	side == 2 ? dir.y = YSCREEN - 200 : 0;
	side == 3 ? dir.x = XSCREEN - 200 : 0;
	side == 3 ? dir.y = 200 : 0;
	a1 = v2.y - v1.y;
	b1 = v1.x - v2.x;
	c1 = a1 * v1.x + b1 * v1.y;
...
static int			nb_outside(t_mytriangle *curr, int side)
{
	...
	else if (side == 1)
	{
		...
		printf("CHeck upside done 0\n");
		curr->vertice[1].y < 200 ? nb = nb | 0x2 : 0;
		printf("CHeck upside done 1\n");
		curr->vertice[2].y < 200 ? nb = nb | 0x4 : 0;
		printf("CHeck upside done 2\n");
	}
	else if (side == 2)
	{
